...
namespace LowPolyWater
{
    public class LowPolyWater : MonoBehaviour
    {
        ...
        public Vector3 GetWaveHeight(Vector3 position)
        {
            ...
            for (int i = 0; i < vertices.Length; i++)
            {
                float distanceX = Mathf.Abs(vertices[i].x - position.x);
                float distanceZ = Mathf.Abs(vertices[i].z - position.z);

                // Суммируем расстояния в обоих направлениях
